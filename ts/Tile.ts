...
export class CitySpec {
  constructor(hasShield: boolean, edges: number[]) {
...
{
  constructor(feature: Feature, cities:number[], shields:Set<number>, roads:number[], farms:number[]) {
  ...
  feature: Feature;
  cities: Map<number,CitySpec>;
  cityForEdge: CitySpec[];
  roads: Map<number,RoadSpec>;
  roadForEdge: RoadSpec[];
  farms: Map<number,FarmSpec>;
  farmForEdge: FarmSpec[];
}
